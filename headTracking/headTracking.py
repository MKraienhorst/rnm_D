...
#parameters from color yaml,!
hdCount = 1 #make to 1 for hd images, =2 for qhd
...
def getHeadPose(depthROIfiltInd, pointCloudHead):
    ...
    y = pointCloudHead[1,:]
    z = pointCloudHead[2,:]
    head3d = (np.dstack((x, y, z)).reshape((-1, 3)).astype(np.float32))
    x = depthROIfiltInd[0,:]
    y = depthROIfiltInd[1,:]
    head2d = (np.dstack((x, y)).reshape((-1, 2)).astype(np.float32))
    ...
    _, rotation_vec1, translation_vec1 = cv.solvePnP(head3d, head2d, camera, distortion, flags=flag)
    _, rotation_vec, translation_vec = cv.solvePnP(head3d, head2d, camera, None, flags=flag)
    ...
    return pose_mat, rotation_vec, translation_vec
...
def testHeadTracking(folderPath):
    ...
    pose_mat, rotation_mat, translation_vec = getHeadPose(depthROIfiltInd, pointCloudHead)
